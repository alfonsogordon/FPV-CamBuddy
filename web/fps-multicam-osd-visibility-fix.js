...
function expOn(){return !!$('fpsExperimentalMaster')?.checked}
...
function repair(){
 ...
 if(panel){
  panel.style.display=multiOn()&&osdOn()?'':'none';
  panel.classList.add('fps-exp-yellow');
 }
...
function addStyle(){
 if($('fpsAdvancedMultiOsdYellowStyle'))return;
 const s=document.createElement('style');s.id='fpsAdvancedMultiOsdYellowStyle';s.textContent=`
...
document.addEventListener('change',e=>{if(['fpsMultiCamSync','fpsExperimentalMaster','fpsOsdMaster','fpsAdvancedMultiOsd'].includes(e.target?.id))queue()},true);
