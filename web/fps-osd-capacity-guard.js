...
function dedupeSameSourceIds(rendered,tpl){if(!advanced())return rendered;const specs=(String(tpl||'').match(TOKEN_RE)||[]).map(parse).filter(p=>p.source>0);if(specs.length<2)return rendered;const sources=[...new Set(specs.map(p=>p.source))];if(sources.length!==1)return rendered;const mode=specs[0].mode||idMode(),cam=cams().find(c=>Number(c.number)===sources[0]);const rawId=mode==='t'?(String(cam?.label||'').trim()||('C'+sources[0])):('C'+sources[0]);const esc=rawId.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');let seen=false;return String(rendered).replace(new RegExp('-'+esc+'(?=\\s|$)','g'),m=>{if(!seen){seen=true;return m}return''}).replace(/\s+/g,' ').trim()}
function lengthFor(tpl){const rendered=String(tpl||'').replace(TOKEN_RE,t=>tokenValue(t));return dedupeSameSourceIds(rendered,tpl).replace(/\s+/g,' ').trim().length}
