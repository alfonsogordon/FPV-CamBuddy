...
function tokenLen(tok){const p=parse(tok);if(!advanced()||p.source===null)return simpleLen(p.base);let n=advancedBaseLen(p.base,p.source);return p.source>0?n+1+identifierLen(p.source,p.mode||idMode()):simpleLen(p.base)}
...
document.addEventListener('input',e=>{if(e.target.matches?.('.fps-cam-label-edit input,#fpsPreviewBattery,#fpsPreviewRemain'))setTimeout(refreshAll,0);const b=e.target.closest?.('.fps-builder');if(b){normalizeAuto(inputFor(b));setTimeout(()=>refresh(b),0)}},true);
