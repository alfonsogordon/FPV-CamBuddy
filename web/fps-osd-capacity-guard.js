...
function literalLen(tpl){return String(tpl||'').replace(TOKEN_RE,'').replace(/\s+/g,' ').trim().length}
function lengthFor(tpl){const s=String(tpl||''),a=s.match(TOKEN_RE)||[];if(!a.length)return s.replace(/\s+/g,' ').trim().length;return a.reduce((n,t)=>n+tokenLen(t),0)+literalLen(s)}
