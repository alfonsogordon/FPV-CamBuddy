...
let boardRaw='',boardDisplay='',pending=false,warned=false,timer=0;
...
function clear(){boardRaw='';boardDisplay='';pending=false;warned=false;clearTimeout(timer);const b=$('fpsVersionWarning');if(b){b.classList.remove('show');b.innerHTML=''}$('fpsVersionDialog')?.remove()}
...
function receive(line){const m=String(line||'').match(/^\[cfg\]\s+FreeCLinker firmware v([^\s]+)/i);if(!m)return false;boardRaw=m[1].trim();boardDisplay=normalise(boardRaw);pending=false;clearTimeout(timer);render();popup();console.info(`[version] board=${boardRaw} display=${boardDisplay} expected=${EXPECTED}`);return true}
function query(){if(!$('connectBtn')?.disabled)return;pending=true;warned=false;boardRaw='';boardDisplay='';sendCommand('version');clearTimeout(timer);timer=setTimeout(()=>{pending=false},1800)}
...
function init(){style();bar();hook();const connect=$('connectBtn'),disconnect=$('disconnectBtn');connect?.addEventListener('click',()=>setTimeout(()=>{hook();if(connect.disabled)query()},700));disconnect?.addEventListener('click',()=>setTimeout(clear,50));window.fpsFirmwareVersion={expected:EXPECTED,legacy:LEGACY,receive,query,clear}}
