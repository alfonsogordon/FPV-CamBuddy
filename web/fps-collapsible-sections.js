...
const STORE='fpsCollapsedMenusV7';
...
function addChevron(host,key,getTargets,toggle,root,opts={}){
 ...
 const {defaultCollapsed=false,followToggle=true}=opts;
 ...
 if(toggle){toggle.addEventListener('change',()=>{if(followToggle){collapsed=!toggle.checked;persist()}draw()})}
 draw();
}
function slug(v){return String(v||'settings').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}
function enhanceOsd(){
 const master=document.getElementById('fpsOsdMaster'),top=master?.closest('.fps-top'),card=top?.closest('.config-card'),masterRow=top?.querySelector(':scope > .fps-master');
 if(!master||!top||!card||!masterRow||card.dataset.fpsOsdCollapse==='1')return;
 ...
 let host=masterRow.querySelector(':scope > .fps-osd-collapse-host');if(!host){host=document.createElement('span');host.className='fps-osd-collapse-host';const sw=master.closest('.switch');if(sw)sw.insertAdjacentElement('afterend',host);else masterRow.appendChild(host)}
 addChevron(host,'osd-main',()=>{
  const version=top.querySelector(':scope > .fps-version');
  const rest=[...card.children].filter(x=>x!==top&&x!==card.querySelector(':scope > .card-header'));
  return [version,...rest,document.getElementById('fpsIntegratedPreview')].filter(Boolean);
 },master,card,{defaultCollapsed:false,followToggle:false});
}
...
function enhanceSection(sec){const head=sec?.querySelector(':scope > .fps-section-head'),body=sec?.querySelector(':scope > .fps-section-body'),toggle=head?.querySelector('input[type="checkbox"]');if(!sec||!head||!body||sec.dataset.fpsMenuCollapse==='1')return;sec.dataset.fpsMenuCollapse='1';sec.classList.add('fps-menu-collapse-root');addChevron(head,sectionKey(sec,toggle),()=>[body],toggle,sec,{defaultCollapsed:false,followToggle:true})}
function enhanceCard(card){if(!card||card.dataset.fpsMenuCollapse==='1'||card.id==='fpsIntegratedPreview')return;const head=card.querySelector(':scope > .card-header');if(!head)return;const title=head.querySelector('h1,h2,h3,strong')?.textContent||head.textContent||'settings';card.dataset.fpsMenuCollapse='1';card.classList.add('fps-menu-collapse-root');addChevron(head,'card-'+slug(title),()=>[...card.children].filter(x=>x!==head),null,card,{defaultCollapsed:false,followToggle:false})}
function enhanceModeSwitchByText(){
 for(const root of document.querySelectorAll('.config-card')){
  const head=root.querySelector(':scope > .card-header');if(!head||!/camera\s*mode\s*switch/i.test(head.textContent||'')||root.dataset.fpsModeSwitchCollapse==='1')continue;
  root.dataset.fpsModeSwitchCollapse='1';root.dataset.fpsMenuCollapse='1';root.classList.add('fps-menu-collapse-root');
  addChevron(head,'camera-mode-switch',()=>[...root.children].filter(x=>x!==head),null,root,{defaultCollapsed:false,followToggle:false});break;
 }
}
...
.fps-menu-collapse-arrow{margin-left:auto!important;width:30px!important;height:30px!important;min-width:30px!important;padding:0!important;border:0!important;background:transparent!important;position:relative!important;border-radius:7px!important;box-shadow:none!important}.fps-menu-collapse-arrow:hover{background:rgba(124,58,237,.12)!important}.fps-menu-collapse-arrow::before{content:'';position:absolute;left:10px;top:8px;width:8px;height:8px;border-right:2px solid #b8a7e8;border-bottom:2px solid #b8a7e8;transform:rotate(45deg);transition:transform .16s ease,top .16s ease,border-color .16s ease}.fps-menu-collapse-arrow.is-collapsed::before{transform:rotate(-45deg);top:10px}.fps-menu-enabled .fps-menu-collapse-arrow::before{border-color:#c4b5fd}.card-header,.fps-section-head,.fps-master{gap:8px}.fps-osd-collapse-host{display:inline-flex;align-items:center;flex:0 0 auto;margin-left:2px}.fps-osd-collapse-host .fps-menu-collapse-arrow{margin-left:0!important}
