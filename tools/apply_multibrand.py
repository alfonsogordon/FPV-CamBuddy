...
# Give the coordinator ownership of BLE scan permission so the protocol
# backends cannot fight over the single ESP32 scanner.
p = "src/camera.h"
s = read(p)
if "void setScanEnabled(bool v)" not in s:
    s = repl(
        s,
        ...
        "camera scan gate setter",
    )
if "_scanEnabled" not in s.split("protected:", 1)[-1]:
    s = repl(
        s,
        "    bool            _lowPowerMode = true;\n",
        "    bool            _lowPowerMode = true;\n"
        "    bool            _scanEnabled  = true;\n",
        "camera scan gate state",
    )
write(p, s)

scan_sigs = {
    "src/ble_camera.cpp": "void BLECamera::startScan() {",
    "src/gopro_camera.cpp": "void GoProCamera::startScan() {",
    "src/sony_camera.cpp": "void SonyCamera::startScan() {",
    "src/blackmagic_camera.cpp": "void BlackmagicCamera::startScan() {",
    "src/insta360_camera.cpp": "void Insta360Camera::startScan() {",
    "src/multi_gopro_camera.cpp": "void MultiGoProCamera::startScan(uint8_t slot) {",
}
for p, sig in scan_sigs.items():
    s = read(p)
    if sig not in s:
        raise SystemExit(f"missing startScan signature in {p}")
    guarded = sig + "\n    if (!_scanEnabled) return;"
    if guarded not in s:
        s = s.replace(sig, guarded, 1)
    write(p, s)

Path("src/multi_camera_coordinator.h").write_text(r'''#pragma once
#include <Arduino.h>
#include "camera.h"
...
Path("src/multi_camera_coordinator.cpp").write_text(r'''#include "multi_camera_coordinator.h"
#include "config.h"
...
p = "src/main.cpp"
s = read(p)
if '#include "multi_camera_coordinator.h"' not in s:
    s = repl(
        s,
        '#include "multi_gopro_camera.h"\n',
        '#include "multi_gopro_camera.h"\n#include "multi_camera_coordinator.h"\n',
        "main coordinator include",
    )
if "static MultiCameraCoordinator multiCameraCoordinator" not in s:
    s = repl(
        s,
...
start = s.find("    const uint8_t camType = configManager.config().cameraType;")
end = s.find('    DBG_SERIAL.printf("[main] Camera type: %s\\n", camTypeName);', start)
if start < 0 or end < 0:
    raise SystemExit("missing main active camera selection boundaries")
new_selection = r'''    const uint8_t camType = configManager.config().cameraType;
...
# Update experimental UI wording while keeping the setting key and feature gate.
for p in [
    "web/config.html",
    "web/fps-ui-rebuild.js",
    "web/fps-stage3-parity.js",
    "web/fps-autosync.js",
]:
    s = read(p)
    for a, b in [
        ("connect/control up to two GoPros", "connect/control multiple cameras across supported brands"),
        ("connect and control up to two GoPros", "connect and control multiple cameras across supported brands"),
        ("up to two GoPros", "multiple supported cameras"),
        ("GoPro Multi Cam", "Multi Brand / Multi Cam"),
    ]:
        s = s.replace(a, b)
    write(p, s)

Path(".github/workflows/apply-v102-expansion.yml").unlink(missing_ok=True)
Path("tools/apply_multibrand.py").unlink(missing_ok=True)
